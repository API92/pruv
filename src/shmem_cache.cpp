...
shmem_cache::shmem_cache(bool for_write) : _for_write(for_write) {}

shmem_cache::~shmem_cache()
{
    _name_to_buf.clear([](hash_table::entry *base_e) {
...
shmem_buffer * shmem_cache::get(char const *name) noexcept
{
    ...
    hash_table::iterator it = _name_to_buf.find(cstr_hash(name, name_len),
    ...
    if (!e->shm.open(e->name, _for_write))
        return nullptr;
